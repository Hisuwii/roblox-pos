...
import { Plus, Pencil, Trash2, Package, PackagePlus, X, Star } from 'lucide-react'
import { supabase } from '../lib/supabase'

const GAMES = ['Blox Fruit', 'Steal a Brainrot', 'Other']
const MAX_QTY = { 'Blox Fruit': 4 }
const EMPTY_FORM = { name: '', price: '', qty: '', category: 'Blox Fruit', image_url: '', is_one_time: false }
...
export default function ItemCatalog() {
  ...
  const ALL_FILTERS = ['All', ...GAMES, 'One-Time Offer']
  const displayed = items.filter((i) => {
    if (gameFilter === 'All') return true
    if (gameFilter === 'One-Time Offer') return i.is_one_time
    return i.category === gameFilter && !i.is_one_time
  })
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-5 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
        {ALL_FILTERS.map((game) => {
          const active = gameFilter === game
          const count = game === 'All'
            ? items.length
            : game === 'One-Time Offer'
            ? items.filter((i) => i.is_one_time).length
            : items.filter((i) => i.category === game && !i.is_one_time).length
          const isOTO = game === 'One-Time Offer'
          return (
            <button
              key={game}
              ...
              style={{
                background: active ? (isOTO ? '#1c1200' : '#1c1c2e') : 'transparent',
                border: active ? `1px solid ${isOTO ? '#d97706' : '#7c3aed'}` : '1px solid #24243c',
                color: active ? (isOTO ? '#fbbf24' : '#a78bfa') : '#4b5563',
              }}
            >
              {isOTO && <Star size={11} className="inline mr-1.5 mb-0.5" />}{game}
              <span className="ml-2 text-xs" style={{ color: active ? (isOTO ? '#d97706' : '#6d28d9') : '#2e2e4a' }}>{count}</span>
            </button>
          )
        })}
      </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-3">
          {displayed.map((item) => (
            <div
              ...
              style={item.is_one_time ? { ...S.card, border: '1px solid #92400e' } : S.card}
              onMouseEnter={(e) => (e.currentTarget.style.borderColor = item.is_one_time ? '#d97706' : '#2e2e4a')}
              onMouseLeave={(e) => (e.currentTarget.style.borderColor = item.is_one_time ? '#92400e' : '#24243c')}
            >
              <div className="relative">
                ...
                {item.is_one_time && (
                  <div className="absolute top-2 left-2 flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold" style={{ background: '#1c1200', color: '#fbbf24', border: '1px solid #92400e' }}>
                    <Star size={9} /> 1× Only
                  </div>
                )}
              </div>
              <div className="p-3 flex flex-col gap-2 flex-1">
                <div>
                  ...
                  <p className="text-xs mt-0.5" style={{ color: '#3d3d60' }}>{item.is_one_time ? 'One-Time Offer' : item.category}</p>
                </div>
                ...
                <div className="flex gap-1.5">
                  ...
                  <button onClick={() => openEdit(item)} className={`flex items-center justify-center px-2 py-1.5 rounded-lg text-slate-400 hover:text-white transition-colors ${item.is_one_time ? 'flex-1' : ''}`} style={{ background: '#1c1c2e' }} title="Edit">
                    <Pencil size={11} />
                  </button>
      ...
      {modalOpen && (
        <Overlay onClose={() => setModalOpen(false)}>
          <div className="flex items-center justify-between mb-5">
            <h3 className="text-white font-bold text-lg">{editing ? 'Edit Item' : 'New Item'}</h3>
            <button onClick={() => setModalOpen(false)} className="flex items-center justify-center w-7 h-7 rounded-lg text-slate-500 hover:text-white transition-colors" style={{ background: '#1c1c2e' }}>
              <X size={15} />
            </button>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <Field label="Type">
              <div className="grid grid-cols-2 gap-2">
                {[
                  { value: false, label: 'Regular', icon: null },
                  { value: true, label: 'One-Time Offer', icon: <Star size={11} /> },
                ].map(({ value, label, icon }) => {
                  const active = form.is_one_time === value
                  const isOTO = value === true
                  return (
                    <button
                      key={String(value)}
                      type="button"
                      onClick={() => setForm({ ...form, is_one_time: value, qty: value ? '1' : form.qty })}
                      className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-sm font-medium transition-all"
                      style={{
                        background: active ? (isOTO ? '#1c1200' : '#1c1c2e') : 'transparent',
                        border: active ? `1px solid ${isOTO ? '#d97706' : '#7c3aed'}` : '1px solid #2e2e4a',
                        color: active ? (isOTO ? '#fbbf24' : '#a78bfa') : '#4b5563',
                      }}
                    >
                      {icon}{label}
              ...
              {form.is_one_time && (
                <p className="text-xs mt-2" style={{ color: '#92400e' }}>This item will be deleted from the catalog once sold.</p>
              )}
            </Field>

            {!form.is_one_time && (
              <Field label="Game">
                <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value, qty: '' })} style={{ background: '#0e0e1a', border: '1px solid #2e2e4a', color: '#e8e8f0', borderRadius: '0.75rem', padding: '0.625rem 0.75rem', width: '100%', fontSize: '0.875rem', outline: 'none' }}>
            ...
            <Field label="Item Name *">
              <input
                style={S.input}
                placeholder={form.is_one_time ? 'e.g. Custom account for @buyer' : 'e.g. Leopard Fruit'}
                value={form.name}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="Stock">
                {form.is_one_time ? (
                  <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm" style={{ background: '#0e0e1a', border: '1px solid #92400e', color: '#fbbf24' }}>
                    <Star size={12} /> 1 <span className="text-xs" style={{ color: '#78350f' }}>(fixed)</span>
                  </div>
                ) : (
                  <input
                    type="number" min="0" max={MAX_QTY[form.category] ?? undefined}
                    style={S.input}
                    placeholder={MAX_QTY[form.category] ? `1–${MAX_QTY[form.category]}` : '0'}
                    value={form.qty}
                    onChange={(e) => {
          ...
          <div className="flex gap-3 mt-6">
            <button onClick={() => setModalOpen(false)} className="flex-1 py-2.5 rounded-xl text-sm font-medium text-slate-300 hover:text-white transition-colors" style={{ background: '#1c1c2e' }}>
              Cancel
            </button>
            <button
              onClick={handleSave}
              ...
              style={{ background: form.is_one_time ? 'linear-gradient(135deg, #d97706, #b45309)' : 'linear-gradient(135deg, #7c3aed, #6366f1)' }}
            >
      ...
      {/* Restock Modal */}
      {restockItem && (
        <Overlay onClose={() => setRestockItem(null)}>
          <div className="flex items-center justify-between mb-5">
            <h3 className="text-white font-bold">Restock</h3>
            <button onClick={() => setRestockItem(null)} className="flex items-center justify-center w-7 h-7 rounded-lg text-slate-500 hover:text-white transition-colors" style={{ background: '#1c1c2e' }}>
              <X size={15} />
            </button>
          </div>
          <div className="flex items-center gap-3 p-3 rounded-xl mb-5" style={{ background: '#1c1c2e' }}>
            <PackagePlus size={18} className="text-violet-400 flex-shrink-0" />
            <div>
              <p className="text-white text-sm font-semibold">{restockItem.name}</p>
              <p className="text-slate-500 text-xs">
                Current stock: <span className="text-white font-semibold">{restockItem.qty}</span>
                {MAX_QTY[restockItem.category] && <span className="text-slate-600"> / max {MAX_QTY[restockItem.category]}</span>}
              </p>
            </div>
          </div>
          <Field label="Add how many?">
            <input
      ...
      {deleteConfirm && (
        <Overlay onClose={() => setDeleteConfirm(null)} small>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-white font-bold">Delete item?</h3>
            <button onClick={() => setDeleteConfirm(null)} className="flex items-center justify-center w-7 h-7 rounded-lg text-slate-500 hover:text-white transition-colors" style={{ background: '#1c1c2e' }}>
              <X size={15} />
            </button>
          </div>
          <div className="flex items-center gap-3 p-3 rounded-xl mb-5" style={{ background: '#1c0a0a', border: '1px solid #7f1d1d' }}>
            <Trash2 size={16} className="text-red-400 flex-shrink-0" />
            <p className="text-slate-300 text-sm">"{deleteConfirm.name}" will be permanently removed.</p>
          </div>
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: '#141422', border: '1px solid #24243c', maxWidth: small ? '24rem' : '28rem' }}
      >
        {children}
      </div>
    </div>
