...
import { Search, Minus, Plus, ShoppingCart, Package, X, Trash2 } from 'lucide-react'
...
export default function POSScreen() {
  ...
  const [cart, setCart] = useState([])           // [{id, name, category, qty, price, maxQty}]
  ...
  function addToCart(item) {
    ...
    setCart((prev) => {
      ...
      return [...prev, { id: item.id, name: item.name, category: item.category, price: item.price, qty: 1, maxQty: item.qty }]
    })
  }
              ...
              {filtered.map((item) => {
                ...
                return (
                  <button
                    ...
                    style={out ? { ...S.card, opacity: 0.3, cursor: 'not-allowed' } : inCart ? S.cardSelected : S.card}
                    onMouseEnter={(e) => { if (!out && !inCart) Object.assign(e.currentTarget.style, { background: '#1c1c2e', borderColor: '#3d3d60' }) }}
                    onMouseLeave={(e) => { if (!out && !inCart) Object.assign(e.currentTarget.style, { background: '#141422', borderColor: '#24243c' }) }}
                  >
                    {item.image_url ? (
                    ...
                    {inCart && (
                      <div className="absolute top-2 right-2 min-w-5 h-5 px-1.5 bg-violet-500 rounded-full flex items-center justify-center">
